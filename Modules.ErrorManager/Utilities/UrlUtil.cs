...
namespace Unic.SitecoreCMS.Modules.ErrorManager.Utilities
{
    /// <summary>
    ...
    {
        /// <summary>
        /// Resolves the <see cref="Sitecore.Globalization.Language"/> based on the current rawurl. There are several steps to check for a valid language:
        /// <list type="number">
        ...
        public static Language ResolveLanguage()
        {
            ...
            Language.TryParse(Sitecore.Web.WebUtil.ExtractUrlParm("sc_lang", Sitecore.Web.WebUtil.GetRawUrl()), out langContext);
            if (langContext != null && !string.IsNullOrEmpty(langContext.Name))
            {
                return langContext;
            }

            Language.TryParse(Sitecore.Web.WebUtil.ExtractUrlParm("la", Sitecore.Web.WebUtil.GetRawUrl()), out langContext);
            if (langContext != null && !string.IsNullOrEmpty(langContext.Name))
            {
                return langContext;
            }

            Language.TryParse(Sitecore.Web.WebUtil.ExtractLanguageName(Sitecore.Web.WebUtil.GetRawUrl()), out langContext);
            if (langContext != null && !string.IsNullOrEmpty(langContext.Name))
            {
                return langContext;
            }
